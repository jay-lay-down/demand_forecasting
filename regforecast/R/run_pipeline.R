# R/run_pipeline.R
# Main pipeline: ENet -> Granger -> SARIMAX -> Backtest -> GIF -> Excel export

#' Run SARIMAX pipeline with Elastic-Net, Granger, CV, and GIF
#' @param file_path Excel path
#' @param pg_name product group label
#' @param sheet_name sheet name
#' @param forecast_h horizon
#' @param test_end c(year, month) end of in-sample metrics window
#' @param sw_thr Shapiro-Wilk p-value threshold for t-innovations fallback
#' @param backtest_start c(year, month) backtest start
#' @return list with artifacts
#' @export
#' @importFrom forecast auto.arima forecast arimaorder
#' @importFrom stats residuals fitted shapiro.test qt time
#' @importFrom zoo as.yearmon
#' @importFrom openxlsx createWorkbook addWorksheet writeData insertImage writeFormula saveWorkbook
#' @importFrom utils flush.console
run_pipeline <- function(file_path, pg_name = "Snack", sheet_name = "data",
                         forecast_h = 12, test_end = c(2024, 12),
                         sw_thr = 0.05, backtest_start = c(2023, 1)) {
  cat("[1/6] Loading and preprocessing...\n"); utils::flush.console()
  p <- prep_data(file_path, sheet_name, pg_name)
  dt <- p$dt; shift <- p$shift; csi_cols <- p$csi_cols

  cat("[2/6] Elastic-Net selection...\n"); utils::flush.console()
  sel_vars <- select_features(dt, "Va_log", csi_cols)

  cat("[3/6] Granger test...\n"); utils::flush.console()
  gr_res <- data.frame(var = character(), lag = integer(), p.value = numeric())
  best_pv <- 1; best_var <- sel_vars[1]; best_lag <- 1
  for (v in sel_vars) for (l in 1:12) {
    gt <- tryCatch(lmtest::grangertest(dt$Va_log ~ dt[[v]], order = l), error = function(e) NULL)
    if (!is.null(gt)) {
      pv <- gt$`Pr(>F)`[2]
      gr_res <- rbind(gr_res, data.frame(var = v, lag = l, p.value = pv))
      if (!is.na(pv) && pv < best_pv) { best_pv <- pv; best_var <- v; best_lag <- l }
    }
  }
  cat(sprintf(" - Selected exog: %s (lag=%d, p=%.4f)\n", best_var, best_lag, best_pv))
  ...
  cat("[4/6] Fitting SARIMAX...\n"); utils::flush.console()
  model <- forecast::auto.arima(y_ts, xreg = xmain$x_tr, seasonal = TRUE, stepwise = FALSE, approximation = FALSE)
  fcast <- forecast::forecast(model, h = forecast_h, xreg = xmain$x_te)

  d_act <- seq(as.Date(sprintf("%04d-%02d-01", start_year, start_month)), by = "month", length.out = tr_len)
  ...
  # Normalize type levels for plotting (matches GIF legend)
  combined$type[combined$type == "Forecast"] <- "Future Forecast"
  combined$type <- factor(combined$type, levels = c("Actual", "Backtest Forecast", "Future Forecast"))

  cat("[5/6] Backtest and GIF...\n"); utils::flush.console()
  bt_df <- run_backtest_from(
    y_ts = y_ts, ex_ts = ex_ts, shift = shift,
    start_year = backtest_start[1], start_month = backtest_start[2], L = best_lag
  )
  bt_plot_file <- paste0("backtest_", pg_name, "_from_", backtest_start[1], sprintf("%02d", backtest_start[2]), ".png")
  plot_backtest(bt_df, pg_name, bt_plot_file)

  anim <- make_forecast_gif(combined, pg_name, bt_df = bt_df,
                            backtest_start_date = as.Date(sprintf("%04d-%02d-01", backtest_start[1], backtest_start[2])))
  gif_file <- anim$gif; gif_preview <- anim$preview

  cat("[6/6] Diagnostics, metrics, export...\n"); utils::flush.console()
  diag_file <- paste0("diagnostics_", pg_name, ".png")
  plot_diagnostics(model, fcast, combined, diag_file)

  sw <- stats::shapiro.test(stats::residuals(model))
  if (sw$p.value < sw_thr) {
    cat(sprintf(" - Residuals non-normal (p=%.4f) -> t-innovations\n", sw$p.value)); utils::flush.console()
    ...
    p_t <- ggplot2::ggplot(dfp, ggplot2::aes(date, mean)) + ggplot2::geom_line() +
      ggplot2::geom_ribbon(ggplot2::aes(ymin = lo95, ymax = hi95), alpha = 0.2) +
      ggplot2::labs(title = "Forecast with t-innovations (95% PI)") + ggplot2::theme_minimal()
    t_file <- paste0("forecast_t_", pg_name, ".png")
    ggplot2::ggsave(t_file, p_t, width = 7, height = 4, dpi = 300)
    cat(" - t-innovations forecast saved ->", t_file, "\n")
  }

  idx <- time(y_ts) <= zoo::as.yearmon(sprintf("%d-%02d", test_end[1], test_end[2])) &
    time(y_ts) >  zoo::as.yearmon("2022-12")
  acc <- accuracy_tbl(invlog(y_ts[idx], shift), invlog(as.numeric(stats::fitted(model))[idx], shift))
  cv  <- rolling_cv_rmse(y_ts, xreg = xmain$x_tr, h = 12)
  acc <- rbind(acc, data.frame(Metric = "CV_RMSE12", Value = cv))

  wb <- openxlsx::createWorkbook()
  openxlsx::addWorksheet(wb, "Prediction"); openxlsx::writeData(wb, "Prediction", combined)
  ...
  openxlsx::insertImage(wb, "Diagnostics", file = diag_file, startRow = 2, startCol = 2, width = 6, height = 8)
  ...
  out_xlsx <- paste0("Predicted_", pg_name, "_elasticnet.gifsafe.xlsx")
  openxlsx::saveWorkbook(wb, out_xlsx, overwrite = TRUE)
  cat("DONE -> ", out_xlsx, "\n", sep = ""); utils::flush.console()

  invisible(list(
    model = model, combined = combined, accuracy = acc, granger = gr_res,
    gif = gif_file, gif_preview = gif_preview, out = out_xlsx, backtest = bt_df
  ))
}
